...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 27 15:56:58 2023
...
# mi1_range = np.linspace(.80, .88, 3000,False)
# np.random.shuffle(mi1_range)

# ma2_range = np.linspace(.07, .10, 3000,False)
# np.random.shuffle(ma2_range)

# mi2_range = np.linspace(.24, .32, 3000,False)
# np.random.shuffle(mi2_range)


# ma3_range = np.linspace(.05, .08, 3000,False)
# np.random.shuffle(ma3_range)

# mi3_range = np.linspace(.15, .20, 3000,False)
# np.random.shuffle(mi3_range)
...
for i in range(3000):
    # ma_r1, mi_r1, ma_r2, mi_r2, ma_r3, mi_r3  = ma1_range[i], mi1_range[i], ma2_range[i], mi2_range[i], .05, .15
    ph = ct_shepp_logan_2d(ma0_range[i], ma0_range[i], -18, 18, .10, .28, .05, .15)
    
    # print(r1_range[r1_label[i]])
    N, M, theta1, theta2, theta3 = ma0_range[i], ma0_range[i], 0, np.deg2rad(-18), np.deg2rad(18)
    ...
    ma_r1, mi_r1, ma_r2, mi_r2, ma_r3, mi_r3  = .52, .82, .10, .28, .05, .15
    
    # ma_r1, mi_r1, ma_r2, mi_r2, ma_r3, mi_r3  = ma1_range[i], mi1_range[i], ma2_range[i], mi2_range[i], ma3_range[i], mi3_range[i]
...
np.save('Shepp_data_In_External_clean.npy', P_all)
